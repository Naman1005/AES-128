...
vector<uint8_t> AES128::encrypt(const vector<uint8_t> &data) {
    vector<uint8_t> padded = data;
    size_t pad = 16 - (padded.size() % 16);
    padded.insert(padded.end(), pad, static_cast<uint8_t>(pad));
    vector<uint8_t> out(padded.size());
    for (size_t i = 0; i < padded.size(); i += 16) {
        uint8_t block[16];
        copyBlock(block, &padded[i]);
        encryptBlock(block);
        copy(block, block + 16, out.begin() + i);
    }
    return out;
}

vector<uint8_t> AES128::decrypt(const vector<uint8_t> &data) {
    if (data.size() % 16 != 0)
    ...
    if (pad == 0 || pad > 16) throw runtime_error("Invalid padding");
    for (size_t i = out.size() - pad; i < out.size(); ++i) {
        if (out[i] != pad) throw runtime_error("Invalid padding");
    }
...
void AES128::mixColumns(uint8_t *state) {
    for (int i = 0; i < 4; ++i) {
        int idx = i * 4;
        uint8_t a = state[idx], b = state[idx + 1], c = state[idx + 2],
                d = state[idx + 3];
